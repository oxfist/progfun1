 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("sum of zeroes is 0") {
    assert(sum(List(0, 0, 0)) === 0)
  }

  test("max of negative numbers is the closest one to 0") {
    assert(max(List(-2, -8, -20, -1, -5)) === -1)
    assert(max(List(-2, -8, -20, -1, 0)) === 0)
  }
  ...
  test("max of empty list throws NoSuchElementException exception") {
    intercept[NoSuchElementException] {
      max(List())
    }
  }

  test("max of a list with the same number is the same number") {
    assert(max(List(7, 7, 7, 7)) === 7)
  }
}
